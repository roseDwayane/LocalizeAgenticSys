 ...
 *
 * Reads the flag from `process.env` first, then falls back to `.env` via
 * `readEnvFile`. The fallback matters because the host is launched directly
 * (`node dist/index.js` under launchd/systemd) without `.env` loaded into the
 * process environment — the rest of the app reads `.env` through `readEnvFile`,
 * so this flag must too, or the opt-out silently does nothing under the service.
 */
